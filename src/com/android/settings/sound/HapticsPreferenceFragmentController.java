...
public class HapticsPreferenceFragmentController extends AbstractPreferenceController
   ...
   private void updateSettings() {
        ...
        int brightnessIntensity = Settings.Secure.getInt(mContext.getContentResolver(),
                Settings.Secure.BRIGHTNESS_SLIDER_HAPTICS_INTENSITY, 0);
        mBrightnessIntensity.setValue(brightnessIntensity);

        int edgeScrollingIntensity = Settings.Secure.getInt(mContext.getContentResolver(),
                Settings.Secure.EDGE_SCROLLING_HAPTICS_INTENSITY, 0);
        mEdgeScrollingIntensity.setValue(edgeScrollingIntensity);

        int volumeSliderIntensity = Settings.Secure.getInt(mContext.getContentResolver(),
                Settings.Secure.VOLUME_SLIDER_HAPTICS_INTENSITY, 0);
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference == mBackIntensity) {
            ...
            Settings.Secure.putInt(mContext.getContentResolver(),
                    Settings.Secure.BRIGHTNESS_SLIDER_HAPTICS_INTENSITY, (Integer) newValue);
            mBrightnessIntensity.setValue((Integer) newValue);
            vibrate((Integer) newValue);
            return true;
        } else if (preference == mEdgeScrollingIntensity) {
            Settings.Secure.putInt(mContext.getContentResolver(),
                    Settings.Secure.EDGE_SCROLLING_HAPTICS_INTENSITY, (Integer) newValue);
            ...
            Settings.Secure.putInt(mContext.getContentResolver(),
                    Settings.Secure.VOLUME_SLIDER_HAPTICS_INTENSITY, (Integer) newValue);
